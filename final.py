...
def pivot_table(df):
    multi_select = st.multiselect('Choose Columns to Display?',
                               ('Day_of_Week', 'Hour', 'Month', 'District', 'Shooting'), default="Month")
    st.write(multi_select)

    values = [x for x in multi_select]
    st.write(values)
    table = pd.pivot_table(df, values='MONTH', index='OFFENSE_DESCRIPTION', columns=[values.upper()],
...
def pie_chart(crimedf):
    st.markdown(f"<h1 style='text-align: center; color: {color_select};'>Pie Chart of Top 10 Most Frequent Offenses"
                "</h1>", unsafe_allow_html=True)
